...
const Home = () => {
  ...
  const onDragEnd = async (result: DropResult) => {
    ...
    try {
      ...
      const success = await sliderService.updateSliderOrder(orderUpdates);
      if (success) {
  ...
  const handleSaveSlider = async () => {
    ...
    try {
      if (editingSlider) {
        ...
        const created = await sliderService.createSlider(sliderData);
        if (created) {
  ...
  const handleUpdateCoverImage = (
    ...
    setHomeCovers({
      ...homeCovers,
      [type === "projects"
        ? "projects_cover_image"
        : "distribution_cover_image"]: imageUrl,
      updated_at: new Date().toISOString(),
    });
    toast.success(
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Last updated:{" "}
                    {new Date(homeCovers.updated_at).toLocaleDateString()}
                  </p>
                </div>

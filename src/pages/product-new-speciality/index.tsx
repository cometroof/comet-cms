...
const SpecialityProductListPage = () => {
  const navigate = useNavigate();
  const queryClient = useQueryClient();
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [productToDelete, setProductToDelete] = useState<Product | null>(null);

  // Fetch products with is_under_product = true and type = 'speciality'
  const { data: products = [], isLoading } = useQuery({
    queryKey: ["products-speciality"],
    queryFn: async () => {
  ...
  const reorderMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["products-speciality"] });
  ...
  const handleDragEnd = (result: DropResult) => {
    ...
    queryClient.setQueryData(["products-speciality"], items);
  ...
  const handleAddProduct = () => {
    navigate("/dashboard/product-speciality/create");
  };

  const handleEditProduct = (product: Product) => {
    navigate(`/dashboard/product-speciality/edit/${product.id}`);
  };

  const handleViewProduct = (product: Product) => {
    navigate(`/dashboard/product-speciality/${product.id}`);
  };
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              Manage your speciality products. Drag to reorder.
            </p>
          </div>
          <Button
        ...
        {products.length === 0 ? (
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-12">
              ...
              <h3 className="text-lg font-semibold mb-2">
                No speciality products yet
              </h3>
              <p className="text-muted-foreground mb-4 text-center">
                Get started by creating your first speciality product
              </p>
          ...
          <Card>
            <CardHeader>
              <CardTitle>All Speciality Products</CardTitle>
              <CardDescription>
...
export default SpecialityProductListPage;

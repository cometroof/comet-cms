...
const SpecialityProductDetailPage = () => {
  ...
  const { data: product, isLoading: productLoading } = useQuery({
    queryKey: ["speciality-product", id],
    queryFn: async () => {
  ...
  const { data: profiles = [], isLoading: profilesLoading } = useQuery({
    queryKey: ["speciality-product-profiles", id],
    queryFn: async () => {
      const { data, error } = await supabase
  ...
  const handleDragEnd = (result: DropResult) => {
    ...
    queryClient.setQueryData(["speciality-product-profiles", id], items);
  ...
  const handleAddProfile = () => {
    navigate(`/dashboard/product-speciality/${id}/profile/create`);
  };

  const handleEditProfile = (profile: ProductProfile) => {
    navigate(`/dashboard/product-speciality/${id}/profile/${profile.id}/edit`);
  };

  const handleViewProfile = (profile: ProductProfile) => {
    navigate(`/dashboard/product-speciality/${id}/profile/${profile.id}`);
  };
  ...
  const handleEditProduct = () => {
    navigate(`/dashboard/product-speciality/edit/${id}`);
  };
          ...
          <CardHeader>
            <CardTitle>
              <div className="flex items-start gap-10 justify-between">
                <div>Speciality Product Information</div>
                <Button
...
export default SpecialityProductDetailPage;
